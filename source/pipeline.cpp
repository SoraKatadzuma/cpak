...
std::error_code executeInShell(const std::vector<std::string>& arguments) {
    ...
    if (output.length > 0)
        logger->info("{}", output.buf.data());
    if (error.length > 0)
        logger->error("{}", error.buf.data());
    
    return cpak::make_error_code((cpak::errc::values)command.retcode());
}


void copyInterfacePropertiesToTarget(const cpak::BuildTarget& interface,
    ...
    if (interface.search != std::nullopt) {
        if (target.search == std::nullopt)
            target.search = cpak::SearchPaths();
...
gatherCompilationArguments(const cpak::BuildTarget& target) noexcept {
    ...
    cpak::utilities::reserveAndAppendFormatted(arguments, *target.defines, "-D {}");
    if (target.search != std::nullopt) {
        cpak::utilities::reserveAndAppendFormatted(arguments, *target.search->include, "-I {}");
        cpak::utilities::reserveAndAppendFormatted(arguments, *target.search->system, "-isystem {}");
    }

    cpak::utilities::reserveAndAppendFormatted(arguments, *target.libraries, "-l {}");
    return arguments;
}
...
gatherLinkingArguments(const cpak::CPakFile& cpakfile,
    ...
    for (auto& objectPath : objects)
        arguments.emplace_back(objectPath);

    // If we're linking a dependency, we'll automatically add it's binary and library
    // paths to the linking command.
    for (const auto& iter : dependencyCache) {
        const auto& dependency = iter.second;
        arguments.emplace_back(fmt::format("-L {}", dependency.binariesBuildPath().string()));
        arguments.emplace_back(fmt::format("-L {}", dependency.librariesBuildPath().string()));
    }

    if (target.search != std::nullopt)
        cpak::utilities::reserveAndAppendFormatted(arguments, *target.search->library, "-L {}");
...
std::error_code cpak::queueForBuild(const cpak::CPakFile& cpakfile,
                                    const cpak::BuildTarget& target) noexcept {
    if (target.type == TargetType::Interface)
        return make_error_code(errc::success);

    auto [consolidated, result] = constructConsolidatedTarget(cpakfile.targets, target);
    ...
    for (const auto& source : *target.sources) {
        ...
        const auto objectPath = cpakfile.objectsBuildPath() / fmt::format("{}.o", sourcePath.filename().c_str());
        ...
        if (std::filesystem::exists(objectPath)) {
            ...
            if (sourceTime < objectTime)
                continue; // Does not need to be built.
        }
...
std::error_code cpak::queueForBuild(const cpak::CPakFile& cpakfile) noexcept {
    auto logger      = spdlog::get("cpak");
    ...
    for (const auto& dependency : cpakfile.dependencies) {
        ...
        if (iter == dependencyCache.end())
            return make_error_code(errc::failure); // Let the caller handle the error.
...
std::error_code cpak::executeBuild() noexcept {
    auto logger = spdlog::get("cpak");
