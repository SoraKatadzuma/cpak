...
///          are included in the hash of the project build. This make it possible
///          to have multiple builds of the same project with different options,
///          and they remain cached.
struct BuildOption {
...
inline static void validateBuildOptionSchema(const YAML::Node& node) {
    // Validate required fields.
    if (!node["name"])
        throw YAML::Exception(node.Mark(), "Build option is missing a name.");
    else if (!node["name"].IsScalar())
        throw YAML::Exception(node.Mark(), "Build option name must be a string.");
    
    if (!node["value"])
        throw YAML::Exception(node.Mark(), "Build option is missing a value.");
    else if (!node["value"].IsScalar())
        throw YAML::Exception(node.Mark(), "Build option value must be a string.");

    // Validate optional fields.
    if (node["desc"] && !node["desc"].IsScalar())
        throw YAML::Exception(node.Mark(), "Build option desc must be a string.");
}
...
inline void interpolateOptions(std::string&              argument,
...
struct YAML::convert<cpak::BuildOption> {
    static Node encode(const cpak::BuildOption& rhs) {
        Node node;
        node["name"]  = rhs.name;
    ...
    static bool decode(const YAML::Node& node, cpak::BuildOption& rhs) {
