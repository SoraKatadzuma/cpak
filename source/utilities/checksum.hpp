...
struct Checksum {
    ...
    static void
    reset(Checksum& checksum) noexcept {
        ...
        checksum.digest_ = { kDigest1Constant, kDigest2Constant,
                             kDigest3Constant, kDigest4Constant,
                             kDigest5Constant };
    ...
    static void
    update(Checksum& checksum, std::string_view data) noexcept {
        for (const auto& byte : data) update(checksum, byte);
    }
    ...
    static void
    finalize(Checksum& checksum, block_t& result) noexcept {
        digest_t digest;
        std::size_t index = 0;
    ...
    private:
    static void
    finalize(Checksum& checksum, digest_t& result) noexcept {
        ...
        std::memcpy(result.data(), checksum.digest_.data(),
                    5 * sizeof(std::uint32_t));
    }

    static void
    update(Checksum& checksum, std::uint8_t byte) noexcept {
        constexpr std::uint32_t kChunkSize = 64;

        checksum.byteCount_++;
        checksum.chunk_[checksum.blockOffset_++] = byte;
        if (checksum.blockOffset_ != kChunkSize) return;
    ...
    static std::uint32_t
    leftRotate(std::uint32_t value, std::uint32_t count) noexcept {
        return (value << count) | (value >> (32 - count));
    }

    static void
    processChunk(Checksum& checksum) noexcept {
...
inline std::string
checksum(const CPakFile& cpakfile) noexcept {
